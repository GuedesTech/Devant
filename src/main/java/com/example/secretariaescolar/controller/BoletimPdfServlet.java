...
public class BoletimPdfServlet extends HttpServlet {
    ...
    // Caminho da logo dentro do webapp (bate com seu JSP: /pages/assets/logo.png)
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 1=Aluno (ajuste se no seu sistema for diferente)
        if (usuario.getId_tipo_user() != 1) {
            response.sendRedirect(request.getContextPath() + "/pages/login/index.jsp");
            return;
        }

        // 2) Descobrir id_aluno + turma (via id_user)
        ...
        // 3) Buscar médias do boletim (todas as disciplinas)
        ...
        try {
            ...
            writer.setPageEvent(new PdfPageEventHelper() {
                ...
                public void onEndPage(PdfWriter w, Document d) {
                    ...
                    PdfContentByte under = w.getDirectContentUnder(); // fundo/shape atrás
                    PdfContentByte over  = w.getDirectContent();      // textos/imagens por cima
