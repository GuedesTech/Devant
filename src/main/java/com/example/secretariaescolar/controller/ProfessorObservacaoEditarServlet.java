...
public class ProfessorObservacaoEditarServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean ok = obsDAO.atualizarDoProfessor(idObservacao, mensagem, tipo);

            if (ok) {
                response.sendRedirect(request.getContextPath() + "/professor/aluno/observacoes?id_aluno=" + idAluno + "&ok=2");
            } else {
                response.sendRedirect(request.getContextPath() + "/professor/aluno/observacoes?id_aluno=" + idAluno + "&erro=2");
            }
